  ...
  useEffect(() => {
    const handleKeydown = (e) => {
      ...
      if (
        e.code === "ArrowDown" &&
        !isOpen &&
        document.activeElement === selectButtonRef.current
      ) {
        setIsOpen(true);
        setActiveIndex(0);
      }
      ...
      if (activeIndex === undefined && e.code === "ArrowDown") {
        setActiveIndex(0);
      }
      if (activeIndex === undefined && e.code === "ArrowUp") {
        setActiveIndex(options.length - 1);
      }

      const indexWithStartingLetter = options.findIndex(
        (element) => element.label.charAt(0).toLowerCase() === e.key,
      );
      ...
      if ((isOpen && e.code === "Space") || (isOpen && e.code === "Enter")) {
